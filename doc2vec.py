...
def words(text):
    ...
    word = nopunct.split(" ")
    word = [w for w in word if len(w) > 2]  # ignore a, an, to, at, be, ...
    word = [w.lower() for w in word]
    goodwords = [w for w in word if w not in ENGLISH_STOP_WORDS]
    return goodwords
    """
...
def load_articles(articles_dirname, gloves):
    ...
    table = []
    for file in files:
        fname = os.path.basename(os.path.dirname(file)) + "/" + os.path.basename(file)
        content = get_text(file)
        content = content.split("\n")
        title = content[0]
        text = " ".join(content[1:])
        centroid = doc2vec(text, gloves)
        table.append((fname, title, text, centroid))
    return table
...
def recommended(article, articles, n):
    recommend = distances(article, articles)
    recommend = sorted(recommend, key=lambda x: x[0], reverse= False)
    a = []
    for i in recommend[0:n]:
        a.append(i[1])
    return a
